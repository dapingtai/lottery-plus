  ...
  <div id="root" class="content">
    ...
    <div class="footer">
      ...
      <el-button
        ...
        type="text"
        circle
        :icon="showCanvas ? 'el-icon-sunrise' : 'el-icon-sunset'"
        @click="switchCanvas()"
      />
      <el-button
        style="left: 140px;"
...
import beginaudio from '@/assets/bg.mp3';
...
export default {
  ...
  created() {
    ...
    const weightConfig = getData(weightConfigField);
    if (weightConfig) {
      this.$store.commit('setWeightConfig', weightConfig);
    }
  },
  mounted() {
    this.startTagCanvas();
  ...
  methods: {
    ...
    switchCanvas() {
      ...
      if (this.showCanvas) {
        this.$el.querySelector('#rootcanvas').style.visibility = 'visible';
      } else {
        this.$el.querySelector('#rootcanvas').style.visibility = 'hidden';
      }
    },
    startTagCanvas() {
      this.createCanvas();
    ...
    toggle(form) {
      ...
      if (this.running) {
        ...
        const weightConfig = this.$store.state.weightConfig[
          this.$store.state.currentKey
        ];
        const resArr = lotteryHandler(number, wins, num, weightConfig);
        this.resArr = resArr;
